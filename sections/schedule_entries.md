...
* `status` - when omitted, a schedule entry created over the API is published (`active`) immediately. Passing `status: "drafted"` creates it as a draft instead. Note that, unlike messages and documents, schedule-entry drafts are **not** listed by [Get my drafts](drafts.md).
